...
import { CuisineContent } from "./CuisineContent";
import { CuisineCTA } from "./CuisineCTA";
import { CuisineDish } from "./CuisineDish";

type CuisineClientProps = {
  content: CuisineContentModel;
};

export function CuisineClient({ content }: CuisineClientProps) {
  ...
  useEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      if (prefersReducedMotion) {
        gsap.set([eyebrow, words, copy, quote, dishes, ingredients, ctaElement], {
          clearProps: "all",
          opacity: 1,
          y: 0,
        });
      ...
      <div className="cuisine-content">
        <Container>
          <div className="cuisine-content-grid">
            <CuisineContent content={content} headlineWords={headlineWords} />
          </div>
        </Container>
      </div>
